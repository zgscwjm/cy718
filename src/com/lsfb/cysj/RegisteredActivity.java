		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0x123) {
				...
				try {
					...
					switch (num) {
					case 1:
						Toast.makeText(RegisteredActivity.this, "注册失败",
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0x123) {
				...
				try {
					...
					switch (num) {
					case 1:
						Toast.makeText(RegisteredActivity.this, "手机号已注册",
								Toast.LENGTH_LONG).show();
						break;
					case 2:
						time.start();// 开始计时
		...
		btngetVerificationCode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (booleans) {
					...
					Toast.makeText(RegisteredActivity.this, "手机号码输入错误",
							Toast.LENGTH_SHORT).show();
				}
		...
		btnsign.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if ("".equals(etMemberRePassWord.getText().toString().trim())) {
					Toast.makeText(RegisteredActivity.this, "第二次输入密码不能为空",
							Toast.LENGTH_SHORT).show();
					return;
				}
				if (!etMemberPassWord.getText().toString().trim()
						.equals(etMemberRePassWord.getText().toString().trim())) {
					Toast.makeText(RegisteredActivity.this, "两次输入密码不一致，请重新输入",
							Toast.LENGTH_SHORT).show();
					return;
				}
				new Thread() {
