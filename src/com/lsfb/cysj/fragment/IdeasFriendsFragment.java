...
public class IdeasFriendsFragment extends Fragment implements
	...
	Handler handlers = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0x123) {
				...
				String str = msg.obj.toString();
				System.err.println("分页加载的数据：" + str);
			...
			if (msg.what == 0x123) {
				...
				try {
					...
					case 2:
						...
						Map<String, Object> map = list.get(zannum);
						map.put("zan", strzannum);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (rootView == null) {
			rootView = inflater.inflate(R.layout.ideas_friends, container,
					false);
		} else {
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					HttpPost post = new HttpPost(MyUrl.StringChuangFriend);
	...
	private void data() {
		baseAdapter = new BaseAdapter() {
			...
			public View getView(final int position, View v, ViewGroup arg2) {
				...
				holder.imageview.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
						// TODO Auto-generated method stub
						String struid = list.get(position).get("uid")
								.toString();
						if (Integer.parseInt(struid) == IsTrue.userId) {
				...
				if (!list.get(position).get("video").toString().equals("0")) {
					...
					// holder.videoView.setOnErrorListener(videoErrorListener);
					// holder.videoView.setVisibility(View.VISIBLE);
					// System.err.println("播放视频没·····");
					// System.err.println(list.get(position).get("video")
					// .toString());
					//
					// Uri uri = Uri.parse(ImageAddress.Stringchuangyi
					// + list.get(position).get("video").toString());
					//
					// holder.videoView.setMediaController(new MediaController(
					// getActivity()));
			...
			// // 对播放出错进行处理
			// Toast.makeText(getActivity(), "无法播放此格式", Toast.LENGTH_SHORT)
			// .show();
			// return true;
			// }
			// };
...
//		params.height = WindowManager.LayoutParams.WRAP_CONTENT;
	...
	private void init() {
		...
		listview = (XListView) rootView.findViewById(R.id.ideas_friends_list);
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					HttpPost post = new HttpPost(MyUrl.StringChuangFriends);
