...
import com.lsfb.cysj.fragment.ArtPrizeFragment.ViewHolder;
 ...
 *
 */
public class LiteraryAwardFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (rootView == null) {
			rootView = inflater.inflate(R.layout.literary_award, container,
					false);
		} else {
...
//			initdata();
	...
	private void initdata() {
		...
		client.post(MyUrl.yxcguo, params, new JsonHttpResponseHandler(){
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				System.out.println(listmap+"NNNNNN");
		...
		adapter = new BaseAdapter() {
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {
					...
					viewHolder.img = (ImageView) convertView.findViewById(R.id.innovation_award_item_img);
					viewHolder.city = (TextView) convertView.findViewById(R.id.innovation_award_item_head);
					viewHolder.text = (TextView) convertView.findViewById(R.id.innovation_award_item_text);
					viewHolder.num = (TextView) convertView.findViewById(R.id.innovation_award_item_num);
					viewHolder.button = (Button) convertView.findViewById(R.id.innovation_award_item_button);
				...
				if (chuangbs.equals("1")) {
					bitmapUtils.display(viewHolder.img, ImageAddress.Stringchuangyi+listmap.get(position).get("image").toString());
				}else if (chuangbs.equals("2")) {
					bitmapUtils.display(viewHolder.img, ImageAddress.cbit+listmap.get(position).get("image").toString());
				}
				viewHolder.city.setText(listmap.get(position).get("title").toString());
				viewHolder.text.setText(listmap.get(position).get("name").toString());
				viewHolder.num.setText("当前票数:"+listmap.get(position).get("piao").toString());
	...
	private void init() {
		list = (ListView) rootView.findViewById(R.id.literary_award_list);
		text = (TextView) rootView.findViewById(R.id.literary_award_list_text);
		map = new HashMap<String, Object>();
